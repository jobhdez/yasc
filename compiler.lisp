	...
	((application-p exp)
	 (if (or (equalp (first exp) '+)
	     ...
	     (make-prim :op (make-var :v (first exp))
			:exps (mapcar (lambda (x) (parse x)) 
				      (cdr exp)))
	     (make-application-exp :exps (mapcar (lambda (x) (parse x)) 
						 (application-exps exp)))))	
	...
	((set!-p exp)
	 (if (not (atomicp (set!-exp exp)))
	     ...
	     (make-anf-set :var (to-anf (set!-var exp) (+ counter 1))
			   :exp (to-anf (set!-exp exp) (+ counter 1)))))
	((definition-p exp)
	 (make-anf-definition :name (to-anf (definition-name exp) counter)
		              :params (mapcar (lambda (x) (to-anf x counter))
					      (definition-params exp))
		              :exp (to-anf (definition-exp exp) counter)))
	((if-exp-p exp)
	 (make-anf-if :cnd (to-anf (if-exp-cnd exp) counter)
		      :thn (to-anf (if-exp-thn exp) counter)
		      :els (to-anf (if-exp-els exp) counter)))
	((lambda-exp-p exp)
	 (make-anf-lambda :params (mapcar (lambda (x) (to-anf x counter))
...
(defmacro exps-to-anf (name fn1 fn2 fn3)
  `(defun ,name (exps counter &optional (tmps '()) (atms '()))
     (let ((exprs (if (listp exps) exps (list exps))))
       (cond ((null exprs)
	      (,fn1 exprs tmps atms))
	     ((atomicp (first exprs))
	      (,fn2 (cdr exprs) 0 tmps (,fn3 (first exprs) atms)))
	     (t
	      (let ((tmp (make-anf-var :v (concatenate 'string "tmp" (format nil "~a" counter)))))
		(make-let-binding :bindings (list tmp)
				  :exp (to-anf (first exprs) (+ counter 1))
				  :body (,name (cdr exprs) (+ counter 1) tmps (,fn3 tmp tmps)))))))))

(defmacro make-anf-exps (name fn)
  `(defun ,name (exps counter tmps atms)
     (cons (to-anf (first exps) (+ counter 1))
	   (,fn (cdr exps) (+ counter 1) tmps atms))))

(make-anf-exps bgn-atm-exps begin-to-anf)
(make-anf-exps app-atm-exps app-to-anf)


(defun prim-null-thn (exps atms tmps)
  (make-prim :op (make-var :v '+) :exps (append atms tmps)))
(defun make-atms (exps atms)
  (cons exps atms))

(defun begin-null (exps atms tmps)
  '())
(defun begin-null-thn (exps atms tmps)
  (ast-to-anf (first (last exps))))

(defun app-null-thn (exps atms tmps)
  '())

(exps-to-anf prim-to-anf prim-null-thn prim-to-anf make-atms)
(exps-to-anf begin-to-anf begin-null-thn bgn-atm-exps make-atms)
(exps-to-anf app-to-anf app-null-thn app-atm-exps make-atms)
...
(defun to-closure (anf *closures* counter)
  (cond ((anf-bool-p anf)
	...
	((anf-quote-p anf)
	 anf)	
	((anf-set-p anf)
	 (let* ((exp (anf-set-exp anf))		(cls-exp (to-closure exp *closures* counter)))
	...
	((anf-definition-p anf)
	 (let ((fn-name (anf-definition-name anf))
	       (params (anf-definition-params anf))
	       (e1 (anf-definition-exp anf)))
	   (make-anf-definition :name fn-name  :params params
			        :exp (to-closure e1 (+ counter 1)))))
       	((prim-p anf)
	 (make-prim :op (prim-op anf) :exps (mapcar (lambda (x) (anf-to-closure x))
						    (prim-exps anf))))
...
(defun free-variables (vars anf)
  (cond ((anf-if-p anf)
	 (append (free-varibles (anf-if-cnd anf) vars)
	         (free-variables (anf-if-thn anf) vars)
		 (free-variables (anf-if-cnd anf) vars)))
	((prim-p anf)
...
(defun to-select (cls counter)
  (cond ((anf-bool-p cls)
	 (if (equalp (anf-bool-b cls) 'true)
	     (make-movq :e1 (make-immediate :imm (make-anf-num :n 1))
			:e2 (concatenate 'string "tmp" (format nil"~a" counter)))
	      (make-movq :e1 (make-immediate :imm (make-anf-num :n 0))
			:e2 (concatenate 'string "tmp" (format nil "~a" counter)))))
	((anf-num-p cls)
	...
	((anf-if-p cls)
	 (let ((v1 (concatenate 'string "tmp" (format nil "~a" counter))))
	   (append (list (gen-if-cnd (anf-if-cnd cls) counter v1)
	...
	((let-binding-p cls)
	 (let* ((e1 (let-binding-exp cls))
	   ...
	   (cond ((or (anf-num-p e1) (anf-var-p e1) (anf-bool-p e1)) 
		 ...
		 ((anf-if-p e1)
		  (append (list (gen-if-cnd (anf-if-cnd e1) counter v1)
...
(defun gen-if-cnd (cnd counter v1)
  (gen-prim-cmp v1 cnd counter))

(defun gen-if (exp v1 counter)
  (cond ((atomicp exp)
	 (make-movq :e1 exp :e2 v1))	
	((prim-p exp)
	 (cond ((equalp (first (prim-exps exp)) '+)
		(gen-prim-add v1 exp counter))
	       ((equalp (first (prim-exps exp)) '-)
...
(defun normalize-if (exp counter)
  (cond 
    ((if? (cnd exp))
     (cond
       ((listp (cnd (cnd exp)))
        (let* ((tmp (concatenate 'string  "tmp" (format nil "~a" counter)))
               (tmp2 (concatenate 'string "tmp" (format nil "~a" (+ counter 1)))))
          `(let ((,tmp ,(cnd (cnd exp)))) 
             (let ((,tmp2 (if ,tmp ,(thn (cnd exp)) ,(els (cnd exp)))))
               (if ,tmp2 ,(thn exp) ,(els exp))))))
       ((or (equalp (cnd exp) 'true) (equalp (cnd exp) 'false))
        (let* ((tmp (concatenate 'string "tmp" (format nil "~a" (+ counter 1)))))
          `(let ((,tmp ,(cnd exp))) 
             (if ,tmp ,(thn exp) ,(els exp)))))))))
(defun normalize-if* (exp counter)
    (cond 
    ((if? exp)
     (cond
...
;; (if (if (= 1 1) true true) 1 2)

(defun if? (x)
    (and (listp x) (equalp (car x) 'if)))
...
(defun els (x)
    (cadddr x))
;; (if (if (= 1 1) true false) 1 2)
;; (if (if (if (= 1 1) true false) false false) 1 2)
;; =>
;;> (normalize-term '(if (if (if (= 1 1) #t #f) #f #f) 1 2))
;;'(let ((g170658 (= 1 1)))
;;   (let ((g170659 (if g170658 #t #f)))
;;     (let ((g170660 (if g170659 #f #f)))
;;       (if g170660 1 2))))
