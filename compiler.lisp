...
(defmacro to_select (name prim-fn)
  `(defun ,name (cls blk counter)
     (cond ((anf-bool-p cls)
	   ...
	   ((let-binding-p cls)
	    (let* ((e1 (let-binding-exp cls))
		   (b1 (car (let-biding-bindings cls)))
	      ...
	      (cond ((or (anf-num-p e1) (anf-var-p e1) (anf-bool-p e1))
		     (append (list (gen-atomic e1 b1)
				   (to-select body
					      blk (+ counter 1)))))
		    ((prim-op e1)
		     (append (list (,prim-fn b1 (prim-exps e1))
			           (to-select body blk couter))))
		    ((anf-if-p e1)
		     (append (list (gen-if-cnd (anf-if-cnd e1))
				   (gen-if (anf-if-thn e1) b1)
			       	   (gen-if (anf-if-cnd e1) b1))))
		    ((let-binding-p e1)
		     (let ((b2 (first (let-binding-bindings e1))))
		       (append (list (to-select (let-binding-exp e1) blk counter)
				     (gen-mov b1 b2))
			       (to-select (let-binding-body e1) blk counnter)))))))
	   ((prim-p cls)
	    (get-primitive cls)))))
	   
(defun gen-atomic (e1 v1) 
  (if (or (anf-num-p e1) (anf-bool-p e1))
      (make-movq :e (make-immediate :imm e1)
		 :e2 (make-stack-location :stk v1))
      (make-movq :e (make-stack-location :stk e1)
		 :e2 (make-register :reg (make-anf-var :v 'rdi)))))

(defmacro gen-prim (constructor name)
  `(defun ,name (var e1 counter)
     (cond ((and (anf-num-p (second e1)) (anf-num-p (third e1)))
	    (let* ((tmp (concatenate 'string "primtemp" (format nil counter)))
		       (tmp-ast (make-anf-var :v tmp)))
		  (list (gen-atomic (second e1) tmp-ast)
			(,constructor :e1 (third e1)
			              :e2 tmp-ast))))
	   ((and (anf-num-p (second e1)) (anf-var-p (third e1)))
	    (list (make-movq :e (second e1) :e2 (make-register :reg 'rdi))
		  (,constructor :e1 (make-register :reg 'rdi)
			       :e2 (third e1))))
	   ((and (anf-var-p (second e1)) (anf-num-p (third e1)))
	    (list (make-movq :e (third e1) :e2 (make-register :reg 'rdi))
		  (,constructor :e1 (make-register :reg 'rdi)
			        :e2 (second e1))))
	   (t (list (make-movq :e (second e1) :e2 (make-register :reg 'rdi))
		    (,constructor :e1 (make-register :reg 'rdi)
				  :e2 (third e1)))))))
(gen-prim make-addq gen-prim-add)
(gen-prim make-subq gen-prim-sub)
(gen-prim make-compq gen-prim-cmp)
		     
;; todo
;; gen-if-cnd
;; gen-if

(defun gen-if-cnd (cnd blk)
  (if (prim-p cnd) (gen-prim-cmp cnd) (to-select cnd blk counter)))

(defun gen-if (exp v1)
  (cond ((atomic-p exp)
	 (make-movq :e exp :e2 (to-select v1)))
	((prim-p exp)
