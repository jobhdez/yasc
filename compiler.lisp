...
(defun to-select (cls counter)
  (cond ((anf-bool-p cls)
	...
	((anf-if-p cls)
	 (let ((v1 (concatenate 'string "tmp" (format nil counter))))
	   (append (list (gen-if-cnd (anf-if-cnd cls))
			 (gen-if (anf-if-thn cls) v1)
			 (gen-if (anf-if-els cls) v1)))))
	((clos-fn-p cls)
	 "hello")
  ...
  `(defun ,name (var e1 counter)
     (cond ((and (anf-num-p (second e1)) (anf-num-p (third e1)))
	   ...
	   ((and (anf-num-p (second e1)) (anf-var-p (third e1)))
	    (list (make-movq :e1 (second e1) :e2 (make-register :reg 'rdi))
		  (,constructor :e1 (make-register :reg 'rdi)
			       :e2 (third e1))))
	   ((and (anf-var-p (second e1)) (anf-num-p (third e1)))
	    (list (make-movq :e1 (third e1) :e2 (make-register :reg 'rdi))
		  (,constructor :e1 (make-register :reg 'rdi)
			        :e2 (second e1))))
	   (t (list (make-movq :e1 (second e1) :e2 (make-register :reg 'rdi))
		    (,constructor :e1 (make-register :reg 'rdi)
				  :e2 (third e1)))))))
