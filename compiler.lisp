...
(defun parse (exp)
  (cond ((variable-p exp)
	...
	((let-p exp)
	 (make-let-exp :bindings (mapcar (lambda (x)
					   (list (parse (first x)) (parse (second x)))
					   (let-bindings exp)))
...
(defun ast-to-anf (exp counter)
  (cond ((var-p exp)
	...
	((quote-exp-p exp)
	 (make-anf-quote :q (ast-to-anf (quote-exp-exp exp) counter))
	((set!-p exp)
	...
	((if-exp-p exp)
	 (cond ((atomicp (if-exp-cnd exp)) exp) 
		...
		(let ((tmp (concatenate 'string "tmp" (format nil "~a" counter))))
		  (make-let-binding :bindings (list (make-anf-var :v tmp))
				    :exp (ast-to-anf (if-exp-cnd exp) (+ counter 1))
				    :body (make-anf-if :cnd (make-anf-var :v tmp)
